...
5. A temporary post meta key `_telex_gpm_source_venue_term_slug` is stored on each created venue post, recording the original `event-venue` term slug for fallback lookups.
6. **Events are silently skipped** in this pass — they are redirected to a temporary non-public post type (`_telex_gpm_skip`) so the WordPress Importer does not report errors. These throwaway posts are automatically deleted at `import_end`.
...
## The `_telex_gpm_source_venue_term_slug` Meta Key
...
During Pass 2, the plugin needs to find which `gatherpress_venue` post corresponds to a given `event-venue` term slug. The primary lookup is by `post_name` (slug). However, WordPress may modify the slug during `wp_insert_post()` to avoid collisions (e.g., appending `-2`). The `_telex_gpm_source_venue_term_slug` meta serves as a **fallback lookup mechanism**: if the primary slug-based query returns no results, the plugin queries by this meta key.
...
2. **Changing the `post_type`** of Event Organiser events (originally `event`) to `_telex_gpm_skip`, a registered but non-public post type.
3. Because `post_type_exists( '_telex_gpm_skip' )` returns `true`, the WordPress Importer does not report "Invalid post type" errors.
4. **At `import_end`**, all posts of type `_telex_gpm_skip` are permanently deleted via `wp_delete_post( $id, true )`.
...
The linking process:
1. Find the `gatherpress_venue` post by slug (or by `_telex_gpm_source_venue_term_slug` meta as fallback).
...
- **The venue post slug was modified by WordPress.** The `_telex_gpm_source_venue_term_slug` fallback should handle this. If it does not, check whether the meta key exists on the venue post.
...
### `_telex_gpm_source_venue_term_slug` meta still present after import
...
This should not happen with the current implementation. The `_telex_gpm_skip` post type is registered before the import begins. If you see this error, ensure the plugin is active and that no other code is interfering with post type registration during the import.
